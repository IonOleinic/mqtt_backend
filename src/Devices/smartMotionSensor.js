...
class SmartMotionSensor extends Device {
  constructor(deviceData) {
    ...
    this.attributes.auto_off = this.sub_type === 'vibration' ? true : false // to be checked
  ...
  processIncomingMessage(topic, payload, io) {
    ...
    if (this.connection_type === 'zigbee') {
      if (topic === this.attributes.receive_result_topic) {
        ...
        if (
          deviceResult?.Movement !== undefined ||
          deviceResult?.Occupancy !== undefined
        ) {
          const movement = deviceResult?.Movement || deviceResult?.Occupancy
          const oldStatus = this.attributes.status
          if (movement == 'ON' || movement == '1') {
            this.attributes.status = 'ON'
            if (this.attributes.auto_off) {
